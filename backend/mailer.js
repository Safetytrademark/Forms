...
async function sendSafetySubmission({ foremanName, project, submissionType, date, workersOnSite, fields, pdfBuffer, pdfName, photos }) {
  ...
  function renderFieldRow(k, v) {
    ...
    if (Array.isArray(v)) return '';                               // skip complex arrays
    if (typeof v === 'object') return '';                          // skip nested objects
    if (typeof v === 'string' && v.startsWith('data:')) return ''; // skip base64

    const str = String(v).trim();
    if (!str) return '';
    return `<tr>
      <td style="${CELL_LABEL}">${labelOf(k)}</td>
      <td style="${CELL_VALUE}">${str.replace(/\n/g, '<br>')}</td>
    </tr>`;
  }
